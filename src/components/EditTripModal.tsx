...
export default function EditTripModal({ trip, onSave, onClose }: Props) {
  ...
  const handleSave = () => {
    if (!isValid) return
    const date = isMonthly ? `${dateValue}-01` : dateValue
    onSave({
      date,
      km: kmVal,
      vehicleType: 'phev',
  ...
  const entryLabel =
    trip.entryType === 'monthly' ? 'Dati mensili' :
    trip.entryType === 'historical' ? 'Km storici' :
    'Viaggio'
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-gray-800">Modifica {entryLabel}</h2>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {isMonthly ? 'Mese di riferimento' : 'Data'}
            </label>
            {isMonthly ? (
              <input type="month" value={dateValue} onChange={(e) => setDateValue(e.target.value)}
                className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-800 outline-none focus:ring-2 focus:ring-purple-500" />
            ) : (
        ...
        <div className="p-6 pt-0 flex gap-3">
          ...
          <button onClick={handleSave} disabled={!isValid}
            className="flex-1 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-200 disabled:text-gray-400 text-white font-semibold py-3 rounded-xl transition-colors">
            Salva
          </button>
        </div>
      </div>
