...
import { Settings, X, Fuel, Zap, TrendingDown } from 'lucide-react'
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="p-6 space-y-5">
          <div>
            ...
            <div className="space-y-3">
              <Field
                ...
                onChange={(v) => set('gasPricePerLiter', v)}
                step="0.01"
              />
              <Field
                label="Consumo"
                unit="km/l"
                value={form.thermalConsumptionKmL}
                onChange={(v) => set('thermalConsumptionKmL', v)}
                step="0.1"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              <Field
                label="Costo elettricità"
                unit="€/kWh"
                value={form.electricityPriceKwh}
                onChange={(v) => set('electricityPriceKwh', v)}
                step="0.01"
              />
              <Field
                label="Consumo elettrico"
                unit="kWh/100km"
                value={form.electricConsumptionKwh100}
                onChange={(v) => set('electricConsumptionKwh100', v)}
                step="0.1"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <Field
              label="Costo extra auto elettrica"
              unit="€"
              value={form.investmentCost}
              onChange={(v) => set('investmentCost', v)}
              step="100"
            />
